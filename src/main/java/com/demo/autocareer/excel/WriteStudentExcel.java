...
public class WriteStudentExcel {
    ...
    public ByteArrayInputStream writeStudentExcel(List<StudentDTOResponse> students) {
        ...
        try (Workbook workbook = new XSSFWorkbook()) {
            ...
            for (int i = 0; i < students.size(); i++) {
                ...
                row.createCell(7).setCellValue(s.getOrganizationFaculty() != null ? s.getOrganizationFaculty().getFaculty().getFacultyName() : "");
    ...
    public ByteArrayInputStream writeInternSemesterExcel(List<InternSemesterDTOResponse> interns) {
        // String timestamp = LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMdd_HHmmss"));
        // String fileName = "students_" + timestamp + ".xlsx";
        // String filePath = "uploads/listStudent/" + fileName;
        ...
        try (Workbook workbook = new XSSFWorkbook()) {
            ...
            for (int i = 0; i < interns.size(); i++) {
                InternSemesterDTOResponse dto = interns.get(i);
                StudentDTOResponse s = dto.getStudent();
                Row row = sheet.createRow(i + 1);
                row.createCell(0).setCellValue(i + 1);
                row.createCell(1).setCellValue(s.getName());
                row.createCell(2).setCellValue(s.getStudentCode());
